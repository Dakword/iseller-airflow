...
from iseller.utils.etl_helpers import etl_hook_insert_data, etl_transform_api_response, validate_json_schema
...
DAG_NAME = "wb_analytics_cards_blocked"
CONN_ID = Config.CONN_ID
TABLE = "cards_blocked"
SQL_CREATE_TABLE = "./sql/cards_blocked.sql"


@dag(
    DAG_NAME,
    dag_display_name="WB Аналитика - Заблокированные карточки",
    start_date=days_ago(1),
    schedule_interval=timedelta(minutes=125),
    default_args={
        'retries': 2,
    ...
    catchup=False,
    max_active_runs=1,
    tags=['wb', 'analytics', 'cards', 'blocked'],
)
def dag_tasks():
        ...
        def seller_tasks():
            ...
            def etl_tasks(seller_id):
                ...
                def extract_api_data() -> list:
                    api_response = AnalyticsApi(get_seller_token(seller_id)).blocked_cards()
